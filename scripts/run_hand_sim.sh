...
CODE_MSG="$(python -m mouth.cli add-deck-codes-to-list --deck "$DECK" --ygo-env "$YGO_ENV_ROOT" 2>&1)" || true
...
cd "$YGO_ENV_ROOT"
python -m mouth.cli hand-sim --deck "$DECK" --ygo-env "$YGO_ENV_ROOT" "${HAND_SIM_ARGS[@]}"
