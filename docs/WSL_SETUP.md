...
Install **xmake** (build system for ygo-env):
...
| Issue | Fix |
|-------|-----|
| `YGO_ENV_ROOT not a directory` | Run step 2; ensure `vendor/ygo-env` exists and contains built files. |
| `Could not import ygoenv` | Run the Python command from inside `vendor/ygo-env` (the script does this), and ensure `PYTHONPATH` includes both `YGO_ENV_ROOT` and the yapping root. |
| `cards.cdb not found` | Run `make` inside `vendor/ygo-env` to download assets. |
...
| **xmake fails on libffi, zlib, lua, ninja** | Install deps: `sudo apt install -y liblua5.4-dev zlib1g-dev libffi-dev ninja-build cmake build-essential`. Clear failed package cache: `rm -rf ~/.xmake/cache/packages/2603` (or the parent number shown in the error path). Then in `vendor/ygo-env` run `xmake f -m release -y` and `xmake` again. |
| **`make` → externally-managed-environment / pip** | Don’t run plain `make` in ygo-env. Run only `make assets scripts`. Install ygoenv into the project venv: `pip install -e vendor/ygo-env/ygoenv` (with venv activated). |

| **`ModuleNotFoundError: ... ygopro_ygoenv`** | The native extension wasn't built. In `vendor/ygo-env` run: `xmake f -m release -y && xmake`. The `.so` is copied to `ygoenv/ygoenv/ygopro/`. Then run the hand sim again. |
